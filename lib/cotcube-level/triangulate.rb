module Cotcube
  module Level
    def triangulate(
      ...
      interval: ,           # interval and swap_type are only needed if saving / caching of swaps is desired
        ...
        loop do
          ...
          if new_members.empty?
            ...
            if min_members >= 3 and members.size >= 3
              ...
              current_slope[:rating] = current_slope[:raw][1..-2].
                map{ |dot|
                  [ dot - current_slope[:raw][0], current_slope[:raw][-1] - dot].min
                }.
                max
            end
      ...
      analyze = lambda do |swaps|
        swaps.each do |swap|
          ...
          swap[:color]  = ((rat > 150) ? :light_blue : (rat > 80) ? :magenta : (rat > 30) ? :light_magenta : (rat > 15) ? :light_yellow : high ? :green : :red) unless interval==:daily
          ...
          # if 10% of avg_dev is below 5 ticks, we use 5 ticks instead
          unless swap[:avg_dev].zero?
            min_dev = [swap[:avg_dev] / 10.to_f, 5 ].max.ceil
            misses        = swap_base.select{|x| x[:dev] <= min_dev and x[:dev].positive? }.map{|x| x[:miss] = x[:dev]; x}
      ...
      current_results.reject!{|swap| swap[:rating] < min_rating or swap[:length] < min_length}
